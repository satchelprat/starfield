...
Graphics::Graphics(HWNDKey &key) {
  ...
  if (FAILED(hr = D3D11CreateDeviceAndSwapChain(
                 ...
                 nullptr, 0, D3D11_SDK_VERSION, &sd, &pSwapChain, &pDevice,
  ...
  if (FAILED(hr = pDevice->CreateRenderTargetView(pBackBuffer.Get(), nullptr,
                                                  &pRenderTargetView))) {
    throw CHILI_GFX_EXCEPTION(hr, L"Creating render target view on backbuffer");
  }

  // set backbuffer as the render target using created view
  pImmediateContext->OMSetRenderTargets(1, pRenderTargetView.GetAddressOf(),
                                        nullptr);
  ...
  // create the texture
  if (FAILED(hr = pDevice->CreateTexture2D(&sysTexDesc, nullptr,
                                           &pSysBufferTexture))) {
  ...
  if (FAILED(hr = pDevice->CreatePixelShader(
                 ...
                 sizeof(FramebufferShaders::FramebufferPSBytecode), nullptr,
                 &pPixelShader))) {
  ...
  if (FAILED(hr = pDevice->CreateVertexShader(
                 ...
                 sizeof(FramebufferShaders::FramebufferVSBytecode), nullptr,
                 &pVertexShader))) {
  ...
  const FSQVertex vertices[] = {
      {-1.0f, 1.0f, 0.5f, 0.0f, 0.0f}, {1.0f, 1.0f, 0.5f, 1.0f, 0.0f},
      {1.0f, -1.0f, 0.5f, 1.0f, 1.0f}, {-1.0f, 1.0f, 0.5f, 0.0f, 0.0f},
      {1.0f, -1.0f, 0.5f, 1.0f, 1.0f}, {-1.0f, -1.0f, 0.5f, 0.0f, 1.0f},
  };
...
void Graphics::EndFrame() {
  ...
  pImmediateContext->IASetVertexBuffers(0u, 1u, pVertexBuffer.GetAddressOf(),
                                        &stride, &offset);
  pImmediateContext->PSSetShaderResources(0u, 1u,
                                          pSysBufferTextureView.GetAddressOf());
